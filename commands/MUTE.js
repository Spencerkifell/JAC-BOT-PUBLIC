module.exports = {
    name: 'MUTE',
    description: "Mutes all the students in a specific channel.",
    execute(message){
        ...
        if(message.guild === null)
            ...
            if(!sender.roles.cache.has(teacherRoleID) && !sender.roles.cache.has(adminID))
            {
                ...
                message.author.send("You need to have to have the role: Teacher").then(d_msg => {d_msg.delete({timeout: 20000}); });
            ...
            if(sender.voice.channel !== null)
            {
                ...
                //console.log(temp);
                temp.every(function(member) { member.voice.setMute(true, "Teacher has muted you"); return true; /*console.log(member);*/ });
                temp.every(function(member) { member.send("Warning: A Teacher has MUTED you!");return true;});
                message.reply("Students have been officially MUTED...").then(d_msg => {d_msg.delete({timeout: 3000}); });
                message.delete({timeout: 1000}); //Deletes User Message...
                return;
            }
    }
